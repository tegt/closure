 ...
 *
 *  Code produced as a quick challenge to show off LAMP stack. This 
 *  module uses a path table to produce timely paged results for an 
 *  org chart display of a 20000 node tree.
 *
 *  The form here is acting entirely as a Ajax responder for a jqGrid
 *  object on the display page. Documentation for jqGrid can be found
 *  at: http://www.trirand.com/jqgridwiki/doku.php?id=wiki:options
 *
 ...
 *  to fail. A JS function defined as the loadError option in
 *  index.html's jqGrid parms pops an alert for the user. In a full
...
/* This is the only php file using the database so inline for
 * simplicity http://us1.php.net/manual/en/mysqli.construct.php
 */
$database = new mysqli('db.tegt.com', 'hasoffers', 'hasoff4me', 'hasoffers');
if ($database->connect_errno)
    errOut('Database connection error '.$database->connect_errno.': '.$database->connect_error);

// Obtain and condition GET parameters from jsGrid call
$pageNum = defGet('page', 1);
$pageRows = defGet('rows', 100);
$sortBy = defGet('sidx', 'enum');
$sortOrder = defGet('sord');
if ($sortOrder <> 'desc') 
    $sortOrder = 'asc'; // only two choices

// Quick query for total number of employees
$result = $database->query('SELECT COUNT(*) AS count FROM employees');
errCheck($result);
$totalRows = $result->fetch_object()->count;

// Use total to calc pages and condition current page num
 ...
 *
 *  The assignment here was to work from an preset table so the paths
 ...
 *  were joined to him at any depth. (Less 1 for the path to himself.)
 *
